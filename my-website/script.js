document.addEventListener('DOMContentLoaded', function () {
    ...
    // Elements and input fields for the Credit Score Simulator
    ...
    // Function to calculate and update the credit score
    ...
    // Helper function to update explanations dynamically
    function updateExplanation(elementId, explanation) {
        document.getElementById(elementId).innerText = explanation;
    }

    // Function to add listeners to input elements for live feedback
 ...
 // Handle the guess button click for credit score range guessing
guessButton.addEventListener('click', function () {
    ...
    if (estimatedScore >= lowerBound && estimatedScore <= upperBound) {
        ...
        // Provide hints based on whether the guess was too high or too low
        if (estimatedScore < lowerBound) {
            ...
            // Potential reasons for a lower score
            ...
            // Potential reasons for a higher score
            if (parseInt(creditUtilization.value) <= 30) {
...
// Enhanced Credit Card Comparison Tool with Guaranteed Output
document.getElementById('findCardBtn').addEventListener('click', function () {
    ...
    // Matching logic for expanded card options
    ...
    // If no specific matches, add general recommendations based on selected features
    ...
    // Ensure at least 3 options are displayed, allowing duplicates if necessary
...
document.getElementById("calculateBudgetBtn").onclick = function() {
    ...
    // Data for the Detailed Breakdown Pie Chart
    const detailedData = {
        labels: [
 ...
 const detailedConfig = {
        ...
        plugins: {
            ...
            legend: {
                ...
                labels: {
                    font: { size: 12 },
                    padding: 15,
                    boxWidth: 10 // Adjusts the size of the color box in legend
                }
            },
            tooltip: {
                callbacks: {
            ...
            datalabels: {
                color: '#fff',
                anchor: 'end', // Positions label towards edge of slice
                align: 'start',
                formatter: (value, ctx) => {
                ...
                font: {
                    weight: 'bold',
                    size: 10 // Smaller font size for detailed labels
                },
    ...
    // Data for the Summary Pie Chart (Main Categories)
    const summaryData = {
        labels: [
...
// Render projected savings progress line chart
...
// Helper function to add tips to the list
...
// Add event listener to the Calculate Growth button
...
function calculateInvestmentGrowth() {
    ...
    let futureValue = initialInvestment; // Start with the initial investment as the future value
    let totalContributions = initialInvestment; // Start with initial investment as total contributions
    let yearByYearData = [];
    ...
    // Calculate growth over time, month by month
    for (let year = 1; year <= yearsInvested; year++) {
        let yearStartValue = futureValue;
        let yearlyContribution = 0; // Track only the contributions made in this year

        for (let month = 1; month <= 12; month++) {
            // Add monthly contribution to total contributions and future value
...
// Function to generate a line chart using Chart.js
function generateChart(data) {
    ...
    // Debt Repayment Calculator functionality
    document.getElementById('calculateRepaymentButton').addEventListener('click', calculateRepayment);

    // Function to calculate and display the debt repayment details
    function calculateRepayment() {
        ...
        let months = 0;
        let totalInterest = 0;
        const balances = []; // To store balance for each month

        while (balance > 0) {
            const interest = balance * interestRate;
            totalInterest += interest;
            balance = balance + interest - monthlyPayment;
            balance = balance < 0 ? 0 : balance; // Ensure balance does not go negative
        ...
        // Display the chart with the repayment data
        displayDebtChart(balances);
    }

    // Function to display the debt repayment chart
    ...
    // Function to update tips and resources based on selected debt type
    ...
    // Call updateDebtInfo initially to display the default selected debt type info
    updateDebtInfo();
});